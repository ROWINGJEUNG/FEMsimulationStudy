...
int main(int argc, char** argv)
{
 ...
 //   groot->addObject(linearSolver);
	...
	Node::SPtr  chain = groot->createChild("Chain");
		...
		// mass 3D model에 FEM을 적용하기 위한 Node, mixedPendulam example을 보면 rigid body를 연조직과 함께 사용한다
		...
		// Tetrahedron degrees of freedom
		MechanicalObject3::SPtr DOF2 = sofa::core::objectmodel::New<MechanicalObject3>();
		massFEMNode->addObject(DOF2);
		DOF2->resize(4);
		DOF2->setName("DOF2");
		WriteAccessor<Data<VecCoord3> > x2 = *DOF2->write(VecId::position());
		x2[0] = Coord3(0, 10, 0);
		x2[1] = Coord3(10, 0, 0);
		x2[2] = Coord3(-10 * 0.5, 0, 10 * 0.866);
		x2[3] = Coord3(-10 * 0.5, 0, -10 * 0.866);
		DOF2->showObject.setValue(true);
		DOF2->showObjectScale.setValue(10.);

		// Tetrahedron uniform mass
		UniformMass3::SPtr mass2 = sofa::core::objectmodel::New<UniformMass3>();
		massFEMNode->addObject(mass2);
		mass2->setMass(2);
		mass2->setName("mass2");

		// Tetrahedron constraints
		FixedConstraint3::SPtr constraints2 = sofa::core::objectmodel::New<FixedConstraint3>();
		massFEMNode->addObject(constraints2);
		constraints2->setName("constraints2");
		constraints2->addConstraint(0);

		// Tetrahedron topology
		MeshTopology::SPtr topology2 = sofa::core::objectmodel::New<MeshTopology>();
		massFEMNode->addObject(topology2);
		topology2->setName("topology2");
		topology2->addTetra(0, 1, 2, 3);

		TetrahedronFEMForceField3::SPtr mass_fem = sofa::core::objectmodel::New<TetrahedronFEMForceField3>();
		massFEMNode->addObject(mass_fem);
		mass_fem->setName("mass_fem");
		mass_fem->setMethod("polar");                      // FEM 방법과 상관없이 그대로 두면 된다.
		mass_fem->setUpdateStiffnessMatrix(true);
		mass_fem->setYoungModulus(6);                     // 물리적인 특성 설정 (신장 모델과 다름을 보이기 위해 60으로 설정)
		mass_fem->setPoissonRatio(0.45);                    // 물리적인 특성 설정

			// kidney 3D model을 visualization 하기 위한 Node (visual model과 mapping을 addObject 해주어야 한다. FEM 노드의 하위 노드이다.)
			Node::SPtr mass_model_node = massFEMNode.get()->createChild("mass_model_node");  // 이 계층구조를 잘 이용하면 여러 모델을 다른 물리적 도메인으로 SOFA 프로그램에 삽입할 수 있을 것이다.

			// obj file load and set visual model
			MeshObjLoader::SPtr mass_obj = sofa::core::objectmodel::New<MeshObjLoader>();
			mass_obj->setName("mass_obj");
			mass_obj->setFilename("model/mass.obj");
			mass_obj->load();
			OglModel::SPtr mass_visual = sofa::core::objectmodel::New<OglModel>();
			mass_visual->setName("mass_visual");
			mass_visual->setSrc("", mass_obj.get());
			mass_model_node->addObject(mass_visual);

			// The mapping between the tetrahedron (DOF) and the liver (visual)
			BarycentricMapping3::SPtr mass_mapping = sofa::core::objectmodel::New<BarycentricMapping3>();
			mass_mapping->setModels(DOF2.get(), mass_visual.get());
			mass_mapping->setName("mass_mapping");
			mass_model_node->addObject(mass_mapping);

		//// make collision node for tumor -> 내가 원하는 대로 작동하는 것 같지는 않다.
		sofa::modeling::createCollisionNodeVec3(massFEMNode, DOF2.get(), "model/mass.obj", collisionModelTypes);
